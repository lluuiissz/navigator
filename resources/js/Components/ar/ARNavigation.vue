...
const userMarker = ref(null) // Store user marker for position updates
...
const startARNavigation = async () => {
  try {
    ...
    // A-Frame and AR.js are loaded via CDN in app.blade.php
    // Wait a tick to ensure they are ready
    await new Promise(resolve => setTimeout(resolve, 300))
    
    if (!window.AFRAME) {
      throw new Error('A-Frame not ready yet. Please wait a moment and try again.')
    }
...
const createARElements = () => {
  ...
  props.routePoints.forEach((point, index) => {
    // Create container entity for GPS positioning
    const container = document.createElement('a-entity')
    container.setAttribute('gps-entity-place', `latitude: ${point.lat}; longitude: ${point.lng};`)
    
    // Create arrow entity (cone) as child - this allows rotation independent of GPS
    ...
    arrow.setAttribute('rotation', '0 0 0') // Will be updated by updateARElements
    arrow.setAttribute('scale', '1 1 1')
    arrow.setAttribute('position', '0 0 0') // Ground level
    
    container.appendChild(arrow)
    scene.appendChild(container)
    arrowEntities.value.push(arrow) // Store the arrow, not the container
  ...
  // Apply initial directional rotation to cones
  setTimeout(() => {
    updateARElements()
  }, 100)
}
...
const initializeMiniMap = () => {
  ...
  userMarker.value = L.marker([userPosition.value.lat, userPosition.value.lng], { icon: userIcon })
    .addTo(miniMapInstance.value)

  
  // Add destination marker (red inverted cone)
  ...
  L.marker([props.destination.lat, props.destination.lng], { icon: destinationIcon })
    .addTo(miniMapInstance.value)
...
const updateARElements = () => {
  // Update AR cone rotations to point toward next waypoint
  console.log('🔄 updateARElements called')
  console.log(`  Arrows: ${arrowEntities.value.length}, Route points: ${props.routePoints.length}`)
  
  if (!arrowEntities.value.length || !props.routePoints.length) {
    console.warn('⚠️ No arrows or route points to update')
    return
  }
  
  props.routePoints.forEach((point, index) => {
    const arrow = arrowEntities.value[index]
    if (!arrow) {
      console.warn(`⚠️ Arrow ${index} is null/undefined`)
      return
    }
    
    // Get next point in route (or destination if last point)
    const nextPoint = index < props.routePoints.length - 1 
      ? props.routePoints[index + 1]
      : props.destination
    ...
    // Rotate cone to point toward next waypoint
    // In A-Frame: X=pitch (tilt), Y=yaw (horizontal rotation), Z=roll
    // Cone points up (+Y) by default:
    // 1. Tilt forward 90° on X-axis (makes it point horizontally)
    // 2. Rotate on Y-axis by bearing to point in correct compass direction
    const rotationValue = `90 ${bearing} 0`
    arrow.setAttribute('rotation', rotationValue)
    
    // Verify rotation was applied
    const actualRotation = arrow.getAttribute('rotation')
    console.log(`🎯 Arrow ${index}:`)
    console.log(`   From: (${point.lat.toFixed(6)}, ${point.lng.toFixed(6)})`)
    console.log(`   To: (${nextPoint.lat.toFixed(6)}, ${nextPoint.lng.toFixed(6)})`)
    console.log(`   Bearing: ${bearing.toFixed(1)}°`)
    console.log(`   Set rotation: ${rotationValue}`)
    console.log(`   Actual rotation: ${JSON.stringify(actualRotation)}`)
  })
  
  console.log('✅ Updated AR cone rotations')
}

const updateMiniMap = () => {
  if (!miniMapInstance.value) {
    console.warn('⚠️ updateMiniMap called but miniMapInstance is null')
    return
  }
  
  const newLatLng = [userPosition.value.lat, userPosition.value.lng]
  console.log(`🗺️ Updating minimap to: (${newLatLng[0].toFixed(6)}, ${newLatLng[1].toFixed(6)})`)
  
  // Update map center to user position
  miniMapInstance.value.setView(newLatLng)
  ...
  if (userMarker.value) {
    ...
    console.warn('⚠️ userMarker is null, cannot update position')
  }
}
