...
import FeedbackAuthModal from '../modals/FeedbackAuthModal.vue'
...
const authModalRole = ref('student') // 'student' | 'faculty' | 'visitor'
...
//  student & faculty → open auth modal (allowlist verification)
//  visitor           → open auth modal (quick register, no ID)
const handleRoleSelection = (roleValue) => {
  showGuestModal.value = false
  authModalRole.value  = roleValue
...
// Re-trigger note display once guestInfo becomes available.
// This handles the race where notes were loaded before guestInfo.id was set.
watch(() => guestInfo.value.id, (newId) => {
  if (newId && map.value && localNotes.value.length > 0) {
    debouncedDisplayNotes()
  }
})
...
const initializeMap = async () => {
  try {
    ...
    // populated before the notes watcher triggers displayNotesOnMap().
    ...
    //   console.log('Map is fully loaded and ready')
      ...
      // Load notes from storage NOW (guestInfo is already set at this point)
      // and display them after a short settle delay.
...
const saveFeedback = async () => {
  ...
  // If authenticated, proceed with normal feedback submission
...
const handleAuthSuccess = async (user) => {
  ...
  // Store pending feedback in sessionStorage
  if (pendingFeedback.value) {
    sessionStorage.setItem('pendingFeedback', JSON.stringify(pendingFeedback.value))
    toast.success('Authentication successful! Reloading to submit feedback...')
  } else {
    toast.success('Login successful!')
  }
  
  // Full page reload to refresh CSRF token and auth state
  setTimeout(() => {
    window.location.reload()
  }, 500)
}
  ...
  if (page.props.auth?.user) {
    ...
    if (storedFeedback) {
      try {
        ...
        axios.post('/create/feedback', feedbackData)
          .then(response => {
            toast.success('Feedback submitted successfully! 🎉')
            
            // Optionally add to local feedback list
            const createdFeedback = {
              ...response.data.feedback,
              message: feedbackData.message,
              marker_id: feedbackData.marker_id,
              guest: {
                id: guestInfo.value.id,
                name: guestInfo.value.nickname
              }
          ...
          .catch(error => {
            ...
            if (error.response?.status === 403) {
              toast.error('Your ID is not authorized to submit feedback. Please contact administrator.', { duration: 6000 })
            } else {
    ...
    // Fallback to session storage if not authenticated (Legacy/Guest mode)
    initializeMap()
    console.log(`Welcome back, ${guestInfo.value.nickname}! (Session)`)
  } else {
    // No guest info, just initialize map
    initializeMap()
  }
...
<template>
  <div class="relative flex h-screen w-full">
    <!-- AR Navigation Overlay (full-screen, shown instead of map) -->
    <ARNavigation
      v-if="isARNavigationActive && selectedLocation"
      :destination="{
        lat: selectedLocation.marker.latitude,
        lng: selectedLocation.marker.longitude,
        name: selectedLocation.name,
      }"
      :route-points="currentRoutePoints"
      @ar-stopped="stopARNavigation"
      @destination-reached="() => { stopARNavigation(); toast.success('You have arrived at ' + selectedLocation.name + '!') }"
    />
                ...
                <div class="grid grid-cols-2 gap-3">
                  ...
                  >
                    <img
                      ...
                      @error="$event.target.closest('article').style.display = 'none'"
                    />
...
>
  <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
    ...
    >
      ...
      <!-- User Panel (logout + activity) - shown when authenticated -->
      <div v-if="page.props.auth?.user" class="absolute top-4 right-4 z-30">
        <UserPanel />
      </div>
      <!-- Fallback name pill for anonymous guest sessions -->
      <div v-else-if="isGuestInfoComplete" class="absolute top-4 right-4 z-10">
        ...
        <div v-else-if="guestStep === 2" class="p-6 sm:p-8">
          ...
          <div class="mb-6 space-y-3">
            <button
              ...
              :key="role.value"
              @click="handleRoleSelection(role.value)"
              :class="[
