...
Route::post('/guests', [GuestController::class, 'store'])->name('guest')->middleware('throttle:10,1');
...
Route::get('/verify-id', [RenderController::class, 'verifyId'])->name('verify-id');
Route::get('/register', [RenderController::class, 'register'])->name('register')->middleware('admin.register');
Route::post('/auth/verify-id', [AuthController::class, 'verifyIdNumber'])->name('auth.verify-id');
Route::post('/register', [AuthController::class, 'register'])->name('register.post');
Route::post('/register/visitor', [AuthController::class, 'registerVisitor'])->name('register.visitor');
...
Route::middleware('auth')->group(function () {
    ...
    // Admin Allowlist Routes
    Route::prefix('admin')->group(function () {
        Route::get('/allowlist', [\App\Http\Controllers\Admin\AllowlistController::class, 'index'])->name('admin.allowlist');
        Route::post('/allowlist/upload', [\App\Http\Controllers\Admin\AllowlistController::class, 'uploadCSV'])->name('admin.allowlist.upload');
        Route::delete('/allowlist/{id}', [\App\Http\Controllers\Admin\AllowlistController::class, 'destroy'])->name('admin.allowlist.destroy');
